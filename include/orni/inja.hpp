...
nssv_DISABLE_MSVC_WARNINGS(4455 26481 26472)
    ...
    class basic_string_view {
       public:
        ...
        nssv_constexpr size_type
        find_last_of(CharT const *s, size_type pos = npos) const  // (4)
        {
        ...
        {
            return pos >= size() ? npos
                                 : to_pos(std::find_if(cbegin() + pos, cend(),
                                                       not_in_view(v)));
        }

        nssv_constexpr size_type find_first_not_of(
            CharT c, size_type pos = 0) const nssv_noexcept  // (2)
        {
            return find_first_not_of(basic_string_view(&c, 1), pos);
        }

        nssv_constexpr size_type find_first_not_of(
            CharT const *s, size_type pos, size_type count) const  // (3)
        {
        ...
        nssv_constexpr size_type find_last_not_of(
            CharT c, size_type pos = npos) const nssv_noexcept  // (2)
        {
...
struct LexerConfig {
    ...
    void update_open_chars() {
        ...
        if (open_chars.find(statement_open_force_lstrip[0]) ==
            std::string::npos) {
        ...
        if (open_chars.find(expression_open_force_lstrip[0]) ==
            std::string::npos) {
...
class FunctionStorage {
   ...
   public:
    ...
    void add_callback(nonstd::string_view name, int num_args,
                      const CallbackFunction &callback) {
        function_storage.emplace(
            std::make_pair(static_cast<std::string>(name), num_args),
    ...
    FunctionData find_function(nonstd::string_view name, int num_args) const {
        ...
        if (it != function_storage.end()) {
            ...
            it = function_storage.find(
                std::make_pair(static_cast<std::string>(name), VARIADIC));
            if (it != function_storage.end()) {
                return it->second;
            }
        }
...
struct InjaError : public std::runtime_error {
    ...
    explicit InjaError(const std::string &type, const std::string &message,
        ...
        : std::runtime_error("[inja.exception." + type + "] (at " +
...
struct ParserError : public InjaError {
    explicit ParserError(const std::string &message, SourceLocation location)
        : InjaError("parser_error", message, location) {}
};

struct RenderError : public InjaError {
    explicit RenderError(const std::string &message, SourceLocation location)
        : InjaError("render_error", message, location) {}
};

struct FileError : public InjaError {
    explicit FileError(const std::string &message)
        : InjaError("file_error", message) {}
...
struct Token {
    ...
    explicit constexpr Token(Kind kind, nonstd::string_view text)
        : kind(kind), text(text) {}
...
inline nonstd::string_view slice(nonstd::string_view view, size_t start,
                                 size_t end) {
...
inline std::pair<nonstd::string_view, nonstd::string_view> split(
    nonstd::string_view view, char Separator) {
...
inline SourceLocation get_source_location(nonstd::string_view content,
                                          size_t pos) {
...
class Lexer {
    ...
    again:
        ...
        // check for close
        if (!close_trim.empty() && inja::string_view::starts_with(
                                       m_in.substr(tok_start), close_trim)) {
            state = State::Text;
    ...
    Token scan_number() {
        for (;;) {
            ...
            char ch = m_in[pos];
            // be very permissive in lexer (we'll catch errors when conversion
            // happens)
    ...
    static nonstd::string_view clear_final_line_if_whitespace(
        nonstd::string_view text) {
   ...
   public:
    explicit Lexer(const LexerConfig &config)
        : config(config), state(State::Text), minus_state(MinusState::Number) {}
    ...
    again:
        ...
        switch (state) {
            ...
            case State::Text: {
                ...
                if (inja::string_view::starts_with(open_str,
                    ...
                    if (inja::string_view::starts_with(
                        ...
                        state = State::ExpressionStart;
                               ...
                               open_str, config.statement_open)) {
                    if (inja::string_view::starts_with(
                ...
                nonstd::string_view text =
                    string_view::slice(m_in, tok_start, pos);
                if (must_lstrip) {
            ...
            case State::StatementBody:
                return scan_body(
                    config.statement_close, Token::Kind::StatementClose,
                    config.statement_close_force_rstrip, config.trim_blocks);
    ...
    const LexerConfig &get_config() const { return config; }
};
...
class LiteralNode : public ExpressionNode {
   public:
    ...
    explicit LiteralNode(const nlohmann::json &value, size_t pos)
        : ExpressionNode(pos), value(value) {}
...
class JsonNode : public ExpressionNode {
   public:
    ...
    explicit JsonNode(nonstd::string_view ptr_name, size_t pos)
        : ExpressionNode(pos),
          name(ptr_name),
...
class FunctionNode : public ExpressionNode {
   ...
   public:
    ...
    explicit FunctionNode(nonstd::string_view name, size_t pos)
        : ExpressionNode(pos),
          precedence(8),
...
class IfStatementNode : public StatementNode {
   public:
    ...
    explicit IfStatementNode(bool is_nested, BlockNode *const parent,
                             size_t pos)
...
class StatisticsVisitor : public NodeVisitor {
    ...
    void visit(const JsonNode &) { variable_counter += 1; }
...
struct Template {
    ...
    /// Return number of variables (total number, not distinct ones) in the
    /// template
    int count_variables() {
...
class Parser {
    ...
    void add_json_literal(const char *content_ptr) {
        nonstd::string_view json_text(
            json_literal_start.data(),
            tok.text.data() - json_literal_start.data() + tok.text.size());
        current_expression_list->rpn_output.emplace_back(
            std::make_shared<LiteralNode>(json::parse(json_text),
                                          json_text.data() - content_ptr));
    }
        ...
        while (tok.kind != closing && tok.kind != Token::Kind::Eof) {
            ...
            switch (tok.kind) {
                ...
                case Token::Kind::Id: {
                    ...
                    // Json Literal
                    if (tok.text == static_cast<decltype(tok.text)>("true") ||
                        ...
                        // Operator
                    } else if (tok.text == "and" || tok.text == "or" ||
                               tok.text == "in" || tok.text == "not") {
                        goto parse_operator;

                        // Functions
                    } else if (peek_tok.kind == Token::Kind::LeftParen) {
                        operator_stack.emplace(std::make_shared<FunctionNode>(
                            static_cast<std::string>(tok.text),
                ...
                parse_operator:
                    ...
                    switch (tok.kind) {
                        ...
                        case Token::Kind::GreaterEqual: {
                            operation =
                                FunctionStorage::Operation::GreaterEqual;
                        } break;
                        case Token::Kind::LessThan: {
                        ...
                        case Token::Kind::Times: {
                            operation =
                                FunctionStorage::Operation::Multiplication;
                        } break;
                        case Token::Kind::Slash: {
                    ...
                    auto function_node = std::make_shared<FunctionNode>(
                        operation, tok.text.data() - tmpl.content.c_str());
                    ...
                    while (!operator_stack.empty() &&
                           ((operator_stack.top()->precedence >
                            ...
                            (operator_stack.top()->precedence ==
                                 function_node->precedence &&
                             function_node->associativity ==
                                 FunctionNode::Associativity::Left)) &&
                ...
                case Token::Kind::RightParen: {
                    ...
                    if (!function_stack.empty() &&
                        function_stack.top().second == current_paren_level) {
                        auto func = function_stack.top().first;
                        auto function_data = function_storage.find_function(
                            func->name, func->number_args);
                        if (function_data.operation ==
                            FunctionStorage::Operation::None) {
                            throw_parser_error("unknown function " +
                                               func->name);
                        }
                        func->operation = function_data.operation;
                        if (function_data.operation ==
                            FunctionStorage::Operation::Callback) {
                            func->callback = function_data.callback;
                        }

                        if (operator_stack.empty()) {
                            throw_parser_error("internal error at function " +
                                               func->name);
                        }
    ...
    bool parse_statement(Template &tmpl, Token::Kind closing,
        ...
        if (tok.text == static_cast<decltype(tok.text)>("if")) {
            ...
            // Chained else if
            if (tok.kind == Token::Kind::Id &&
                tok.text == static_cast<decltype(tok.text)>("if")) {
                get_next_token();
            ...
            if (tok.kind == Token::Kind::Comma) {
                ...
                for_statement_node = std::make_shared<ForObjectStatementNode>(
                    static_cast<std::string>(key_token.text),
                    static_cast<std::string>(value_token.text), current_block,
                    tok.text.data() - tmpl.content.c_str());

                // Array type
            } else {
                for_statement_node = std::make_shared<ForArrayStatementNode>(
                    static_cast<std::string>(value_token.text), current_block,
                    tok.text.data() - tmpl.content.c_str());
            }
            ...
            if (tok.kind != Token::Kind::Id ||
                tok.text != static_cast<decltype(tok.text)>("in")) {
            ...
            if (config.search_included_templates_in_files &&
                template_storage.find(pathname) == template_storage.end()) {
            ...
            current_block->nodes.emplace_back(
                std::make_shared<IncludeStatementNode>(
                    pathname, tok.text.data() - tmpl.content.c_str()));
            ...
            if (tok.kind != Token::Kind::Id) {
                throw_parser_error("expected variable name, got '" +
                                   tok.describe() + "'");
            }
            ...
            auto set_statement_node = std::make_shared<SetStatementNode>(
                key, tok.text.data() - tmpl.content.c_str());
        ...
        for (;;) {
            ...
            switch (tok.kind) {
                ...
                case Token::Kind::Text: {
                    current_block->nodes.emplace_back(
                        std::make_shared<TextNode>(
                            tok.text.data() - tmpl.content.c_str(),
                            tok.text.size()));
                } break;
                case Token::Kind::StatementOpen: {
                    get_next_token();
                    if (!parse_statement(tmpl, Token::Kind::StatementClose,
                                         path)) {
                        throw_parser_error("expected statement, got '" +
                                           tok.describe() + "'");
                    }
                    if (tok.kind != Token::Kind::StatementClose) {
                        throw_parser_error("expected statement close, got '" +
                                           tok.describe() + "'");
                    }
                } break;
                case Token::Kind::LineStatementOpen: {
                    get_next_token();
                    if (!parse_statement(tmpl, Token::Kind::LineStatementClose,
                                         path)) {
                        throw_parser_error("expected statement, got '" +
                                           tok.describe() + "'");
                    }
                    if (tok.kind != Token::Kind::LineStatementClose &&
                        tok.kind != Token::Kind::Eof) {
                        throw_parser_error(
                ...
                case Token::Kind::ExpressionOpen: {
                    ...
                    if (!parse_expression(tmpl, Token::Kind::ExpressionClose)) {
                        throw_parser_error("expected expression, got '" +
                                           tok.describe() + "'");
                    }

                    if (tok.kind != Token::Kind::ExpressionClose) {
                        throw_parser_error("expected expression close, got '" +
                                           tok.describe() + "'");
                    }
                } break;
                case Token::Kind::CommentOpen: {
                    ...
                    if (tok.kind != Token::Kind::CommentClose) {
                        throw_parser_error("expected comment close, got '" +
                                           tok.describe() + "'");
                    }
                } break;
                default: {
   ...
   public:
    explicit Parser(const ParserConfig &parser_config,
...
class Renderer : public NodeVisitor {
    ...
    const std::shared_ptr<json> eval_expression_list(
        ...
        if (!result) {
            if (not_found_stack.empty()) {
                throw_renderer_error("expression could not be evaluated",
                                     expression_list);
            }
    ...
    void throw_renderer_error(const std::string &message, const AstNode &node) {
        SourceLocation loc =
            get_source_location(current_template->content, node.pos);
            ...
            if (!result[N - i - 1]) {
                ...
                if (throw_not_found) {
                    throw_renderer_error(
                        "variable '" +
                            static_cast<std::string>(json_node->name) +
                            "' not found",
                        *json_node);
                }
            }
    ...
    void visit(const TextNode &node) {
        output_stream->write(current_template->content.c_str() + node.pos,
                             node.length);
    }

    void visit(const ExpressionNode &) {}

    void visit(const LiteralNode &node) { json_eval_stack.push(&node.value); }
    ...
    void visit(const JsonNode &node) {
        if (json_additional_data.contains(node.ptr)) {
            ...
            if (function_data.operation ==
                ...
                auto value =
                    std::make_shared<json>(function_data.callback(empty_args));
    ...
    void visit(const FunctionNode &node) {
        ...
        switch (node.operation) {
            ...
            case Op::And: {
                ...
                result_ptr =
                    std::make_shared<json>(truthy(args[0]) && truthy(args[1]));
                json_tmp_stack.push_back(result_ptr);
                json_eval_stack.push(result_ptr.get());
            } break;
            case Op::Or: {
            ...
            case Op::In: {
                ...
                result_ptr = std::make_shared<json>(
                    std::find(args[1]->begin(), args[1]->end(), *args[0]) !=
                    args[1]->end());
                json_tmp_stack.push_back(result_ptr);
                json_eval_stack.push(result_ptr.get());
            } break;
            ...
            case Op::Add: {
                ...
                if (args[0]->is_string() && args[1]->is_string()) {
                    ...
                    result_ptr = std::make_shared<json>(args[0]->get<int>() +
                                                        args[1]->get<int>());
                    json_tmp_stack.push_back(result_ptr);
                } else {
                    result_ptr = std::make_shared<json>(args[0]->get<double>() +
                                                        args[1]->get<double>());
                    json_tmp_stack.push_back(result_ptr);
                }
                json_eval_stack.push(result_ptr.get());
            ...
            case Op::Power: {
                ...
                if (args[0]->is_number_integer() && args[1]->get<int>() >= 0) {
                    int result =
                        std::pow(args[0]->get<int>(), args[1]->get<int>());
                    result_ptr = std::make_shared<json>(std::move(result));
                    json_tmp_stack.push_back(result_ptr);
                } else {
                    double result =
                        std::pow(args[0]->get<double>(), args[1]->get<int>());
                    result_ptr = std::make_shared<json>(std::move(result));
                    json_tmp_stack.push_back(result_ptr);
                }
                json_eval_stack.push(result_ptr.get());
            ...
            case Op::DivisibleBy: {
                ...
                result_ptr = std::make_shared<json>(
                    (divisor != 0) && (args[0]->get<int>() % divisor == 0));
                json_tmp_stack.push_back(result_ptr);
                json_eval_stack.push(result_ptr.get());
            } break;
            case Op::Even: {
                result_ptr = std::make_shared<json>(
                    get_arguments<1>(node)[0]->get<int>() % 2 == 0);
                json_tmp_stack.push_back(result_ptr);
                json_eval_stack.push(result_ptr.get());
            } break;
            case Op::Exists: {
                auto &&name =
                    get_arguments<1>(node)[0]->get_ref<const std::string &>();
                result_ptr = std::make_shared<json>(
                    json_input->contains(json::json_pointer(
                        JsonNode::convert_dot_to_json_ptr(name))));
                json_tmp_stack.push_back(result_ptr);
                json_eval_stack.push(result_ptr.get());
            } break;
            case Op::ExistsInObject: {
                ...
                result_ptr = std::make_shared<json>(args[0]->find(name) !=
                                                    args[0]->end());
                json_tmp_stack.push_back(result_ptr);
                json_eval_stack.push(result_ptr.get());
            } break;
            ...
            case Op::Float: {
                result_ptr = std::make_shared<json>(std::stod(
                    get_arguments<1>(node)[0]->get_ref<const std::string &>()));
                json_tmp_stack.push_back(result_ptr);
                json_eval_stack.push(result_ptr.get());
            } break;
            case Op::Int: {
                result_ptr = std::make_shared<json>(std::stoi(
                    get_arguments<1>(node)[0]->get_ref<const std::string &>()));
                json_tmp_stack.push_back(result_ptr);
                json_eval_stack.push(result_ptr.get());
            } break;
            ...
            case Op::Length: {
                ...
                if (val->is_string()) {
                    result_ptr = std::make_shared<json>(
                        val->get_ref<const std::string &>().length());
                } else {
            ...
            case Op::Lower: {
                std::string result =
                    get_arguments<1>(node)[0]->get<std::string>();
            ...
            case Op::Max: {
                auto args = get_arguments<1>(node);
                auto result =
                    std::max_element(args[0]->begin(), args[0]->end());
                json_eval_stack.push(&(*result));
            } break;
            case Op::Min: {
                auto args = get_arguments<1>(node);
                auto result =
                    std::min_element(args[0]->begin(), args[0]->end());
                json_eval_stack.push(&(*result));
            } break;
            case Op::Odd: {
                result_ptr = std::make_shared<json>(
            ...
            case Op::Round: {
                ...
                double result = std::round(args[0]->get<double>() *
            ...
            case Op::IsBoolean: {
                result_ptr = std::make_shared<json>(
                    get_arguments<1>(node)[0]->is_boolean());
                json_tmp_stack.push_back(result_ptr);
                json_eval_stack.push(result_ptr.get());
            } break;
            case Op::IsNumber: {
                result_ptr = std::make_shared<json>(
                    get_arguments<1>(node)[0]->is_number());
                json_tmp_stack.push_back(result_ptr);
                json_eval_stack.push(result_ptr.get());
            } break;
            case Op::IsInteger: {
                result_ptr = std::make_shared<json>(
                    get_arguments<1>(node)[0]->is_number_integer());
                json_tmp_stack.push_back(result_ptr);
                json_eval_stack.push(result_ptr.get());
            } break;
            case Op::IsFloat: {
                result_ptr = std::make_shared<json>(
                    get_arguments<1>(node)[0]->is_number_float());
                json_tmp_stack.push_back(result_ptr);
                json_eval_stack.push(result_ptr.get());
            } break;
            case Op::IsObject: {
                result_ptr = std::make_shared<json>(
                    get_arguments<1>(node)[0]->is_object());
                json_tmp_stack.push_back(result_ptr);
                json_eval_stack.push(result_ptr.get());
            } break;
            case Op::IsArray: {
            ...
            case Op::IsString: {
                result_ptr = std::make_shared<json>(
                    get_arguments<1>(node)[0]->is_string());
                json_tmp_stack.push_back(result_ptr);
                json_eval_stack.push(result_ptr.get());
            } break;
            case Op::Callback: {
    ...
    void visit(const ForObjectStatementNode &node) {
        ...
        for (auto it = result->begin(); it != result->end(); ++it) {
            ...
            json_additional_data[static_cast<std::string>(node.value)] =
                it.value();
    ...
    void visit(const IncludeStatementNode &node) {
        ...
        if (included_template_it != template_storage.end()) {
            sub_renderer.render_to(*output_stream, included_template_it->second,
                                   *json_input, &json_additional_data);
...
class Environment {
   ...
   public:
    ...
    explicit Environment(const std::string &global_path)
        : input_path(global_path), output_path(global_path) {}
    ...
    /// Sets whether to strip the spaces and tabs from the start of a line to a
    /// block
    ...
    Template parse_template(const std::string &filename) {
        ...
        auto result = Template(
            parser.load_file(input_path + static_cast<std::string>(filename)));
        parser.parse_into_template(
            result, input_path + static_cast<std::string>(filename));
        return result;
    }
    ...
    void write(const std::string &filename, const json &data,
    ...
    void write(const Template &temp, const json &data,
               const std::string &filename_out) {
        std::ofstream file(output_path + filename_out);
    ...
    std::ostream &render_to(std::ostream &os, const Template &tmpl,
                            const json &data) {
        Renderer(render_config, template_storage, function_storage)
            .render_to(os, tmpl, data);
        return os;
    }
    ...
    */
    void add_callback(const std::string &name,
                      const CallbackFunction &callback) {
    ...
    */
    void add_void_callback(const std::string &name,
    ...
    */
    void add_callback(const std::string &name, int num_args,
                      const CallbackFunction &callback) {
    ...
    */
    void add_void_callback(const std::string &name, int num_args,
                           const VoidCallbackFunction &callback) {
        function_storage.add_callback(name, num_args,
...
*/
inline void render_to(std::ostream &os, nonstd::string_view input,
                      const json &data) {
    Environment env;
